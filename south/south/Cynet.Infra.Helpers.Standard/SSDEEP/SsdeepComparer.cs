...
{
    public class SsdeepComparer : ISsdeepComparer
    {
        public int CompareMultiple(string ssdeep, string[] ssdeeps)
        {
            int max = 0;

            try
            {
                for (int i = 0; i < ssdeeps.Count(); ++i)
                {
                    var currentSsdeep = ssdeeps[i];
                    var res = Compare(ssdeep, currentSsdeep);
                    if (res > max)
                    {
                        max = res;
                    }
            ...
            {
                throw new ApplicationException(String.Format("error occured during compare multiple ssdeeps with ssdeep:{0}", ssdeep), ex);
            }

            return max;
        }
        public Tuple<int, string> CompareMultiple(string ssdeep, string[] ssdeeps, int? threshold = null)
        {
            int max = 0;
            ...
            try
            {
                for (int i = 0; i < ssdeeps.Count() && !didFindResult; ++i)
                {
                    var currentSsdeep = ssdeeps[i];
                    var res = Compare(ssdeep, currentSsdeep);
                    ...
                    {
                        max = res;
                        similar = currentSsdeep;

                        if (threshold.HasValue && res > threshold.Value)
                        {
                            didFindResult = true;
                        }
                    }
            ...
            return Tuple.Create(max, similar);
        }
        public int Compare(string ssdeep1, string ssdeep2)
        {
            int score = 0;
            ...
            try
            {
                ...
                if (ssdeep1 == null || ssdeep2 == null)
                {
                    throw new ArgumentNullException();
                }

                // each spamsum is prefixed by its block size
                var colon1Pos = ssdeep1.IndexOf(':');
                var colon2Pos = ssdeep2.IndexOf(':');
                if (colon1Pos == -1 ||
                    colon2Pos == -1 ||
                    !Int32.TryParse(ssdeep1.Substring(0, colon1Pos), out block_size1) ||
                    !Int32.TryParse(ssdeep2.Substring(0, colon2Pos), out block_size2) ||
                    block_size1 < 1 || block_size2 < 1)
                {
                    return 0;//throw new ApplicationException("Badly formed input");
                }
                ...
                {
                    return 0;//throw new ApplicationException("Given signatures cannot be compared");
                }

                var colon12Pos = ssdeep1.IndexOf(':', colon1Pos + 1);
                var colon22Pos = ssdeep2.IndexOf(':', colon2Pos + 1);
                ...
                // about the filename
                var comma1Pos = ssdeep1.IndexOf(',', colon12Pos + 1);
                var comma2Pos = ssdeep2.IndexOf(',', colon22Pos + 1);

                s1_1 = ssdeep1.ToCharArray(colon1Pos + 1, colon12Pos - colon1Pos - 1);
                s2_1 = ssdeep2.ToCharArray(colon2Pos + 1, colon22Pos - colon2Pos - 1);

                s1_2 = ssdeep1.ToCharArray(colon12Pos + 1, comma1Pos == -1 ? ssdeep1.Length - colon12Pos - 1 : comma1Pos - colon12Pos - 1);
                s2_2 = ssdeep2.ToCharArray(colon22Pos + 1, comma2Pos == -1 ? ssdeep2.Length - colon22Pos - 1 : comma2Pos - colon22Pos - 1);
            ...
            {
                throw new ApplicationException(String.Format("error occured during ssdeep compare (ssdeep1:{0} ssdeep2:{1})", ssdeep1, ssdeep2));
            }
